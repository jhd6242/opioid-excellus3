...
renderTable({
  ...
  colnames(facility_visits_df)<-c('Year','County',"Geography" ,'Payer','ER visit-Opioid', 'Total ER visit','Opioid ER Visit rate per 1000')
  
  print(head(facility_visits_df[(facility_visits_df$County %in% c('Albany','Bronx') & facility_visits_df$Year=='2015') ,c('Year','County', 'Payer','ER visit-Opioid', 'Total ER visit','Opioid ER Visit rate per 1000')],10))
  })
```
...
\[
\begin{eqnarray}
Opioid\ ER\ Visit\ Rate\ for\ a\ Commercial\ payer\ for\ a\ County\ _{Year}& = & \dfrac {No.\ of\ ER\ visits\ for\ opioid\ abuse\ for\ Commercial\ Payer\ _{Year}\  } {Total\ ER\ visits\ for\ that\ county\ _{Year}}\hspace{10mm} *\ \ 1000\\
\end{eqnarray}
\]
...
\[
\begin{eqnarray}
Opioid\ ER\ Visit\ Rate\ for\ a\ Commercial\ payer\ for\ Albany\ _{2015}& = & \frac {37}{36,091}\ *\ 1000\\& \\& \approx & 1.02
\end{eqnarray}
\]
...
\[
\begin{eqnarray}
Opioid\ ER\ Visit\ Rate\ across\ all\ the\ payer\ for\ a\ County\ _{Year}& = & \dfrac {\sum\limits_{i\ \in\  (Commercial,\  Medicaid,\  Medicare,\ Professional,\ Other\ )}\ No.\ of\ ER\ visits\ for\ opioid\ abuse\ _{i,\ Year\ }\  } {\sum\limits_{i\ \in\  (Commercial,\  Medicaid,\  Medicare,\ Professionl,\ Other\ )}\ Total\ ER\ visits\ for\ that\ county\ _{i,\ Year}} *\ \ 1000\\
\end{eqnarray}
\]
...
\[
\begin{eqnarray}
Opioid\ ER\ Visit\ Rate\ across\ all\ the\ payer\ for\ Albany\ _{Year}& = & \dfrac {37\ +\ 39\ +\ 9\ +\ 3\ +\ 0\ } {(\ 36,091\ +\ 41,299\ +\ 20,079\ +\ 2,384\ +\ 17\ )\ } *\ \ 1000\\& \\& \approx & 0.88
\end{eqnarray}
\]
...
\[
\begin{eqnarray}
Indexed\ Opioid\ ER\ Visit\ Rate\ for\ a\ Commercial\ payer\ for\ a\ County\ _{2015}& = & \frac {Opioid\ ER\ Visit\ Rate\ for\ a\ Commercial\ payer\ for\ a\ County\ _{2015} }{mean\ (Opioid\ ER\ Visit\ Rate\ for\ a\ Commercial\ payer\ across\ all\ the\ Counties\ _{2015} }\ *\ 100\\
\end{eqnarray}
\]
...
\[
\begin{eqnarray}
Indexed\ Opioid\ ER\ Visit\ Rate\ for\ a\ Commercial\ payer\ for\ Albany\ _{2015}& = & \frac {1.02}{(\ 1.02\ +\ 0.26\ )\ /\ 2 }\ *\ 100\\& \approx &\ 79.68
\end{eqnarray}
\]
...
shinyApp(
  ...
  server = function(input, output){
    ...
    output$nplot02 <- renderChart2({
      ...
      x1<- hPlot(x='Year', y=visit()[[2]] ,group='Payer', data= df3, type='line', title=paste0(input$id,' for 2010-2017'), subtitle = 'Business Segment')
      x1$params$width <- 642
      x1$params$height <- 540
      x1
    })
 ...
 output$nplot03 <- renderChart2({
  ...
  x1<- hPlot(`Percent Change`~Year, group='Region', data= df3, type='column', title=paste0('Percent Change in ',input$id,' for 2011-2017'), subtitle = 'For Every Excellus Region ')
  x1$params$width <- 642
  x1$params$height <- 540
  x1
})
  ...
  x1<- hPlot(`Percent Change`~Year, group='Payer', data= df3, type='column', title=paste0('Percent Change in ',input$id,' for 2011-2017'), subtitle = 'For Every Payer')
  x1$params$width <- 642
  x1$params$height <- 540
  x1
    })
...
1.  Geospatial mapping of Excellus counties. It consists of four tabs:
    ...
    1.1. Opioid Epidemic Map: Overall opioid visit rate by grouping each county and payer. This helps to summarize data across multiple years,  multiple payers and different line of business.
