...
visits<- list(
  'Emergency Room'= c('PainandAbuse.ER.visits','Total.ER.visits','PainandAbuse.ER.visit.rate'),
  'Inpatient' =
    c('PainandAbuse.Inpatient.visits','Total.Inpatient.visits','PainandAbuse.Inpatient.visit.rate'),
  'Outpatient' = 
    c('PainandAbuse.Outpatient.visits','Total.Outpatient.visits','PainandAbuse.Outpatient.visit.rate'),
  'Professional' = 
    c('PainandAbuse.Professional.visits','Total.Professional.visits','PainandAbuse.Professional.visit.rate'),
  'Other' = 
    c('PainandAbuse.Other.visits','Total.Other.visits','PainandAbuse.Other.visit.rate')
...
dependency_grouping <- c('PainandAbuse','Diagnosis','PainAbuseDiagnosis')
```
  ...
  fluidRow(
    ...
    absolutePanel(id = "controls", class = "panel panel-default", fixed = TRUE, draggable = TRUE, top = 60, left = "auto", right = 20, bottom = "auto", width = 330, height= "auto",
                  ...
                  selectInput("payer", h4("Choose Line of Business:"), choices= payer, selected = "Commercial",multiple=TRUE, selectize = TRUE),
                   ...
                   selectInput("dependency_grouping", h4("Choose Opioid Grouping:"), choices= dependency_grouping, selected = "PainandAbuse",multiple=FALSE, selectize = TRUE),
      ...
      conditionalPanel(
         condition = "input.dependency_grouping == 'PainandAbuse'",
    ...
    absolutePanel(id = "controls", class = "panel panel-default", fixed = TRUE, draggable = TRUE, bottom = 0 , right = "auto", left = -5, top = "auto", width = 760, height= 10,
                 tags$p(tags$strong(tags$u("Indexed PainandAbuse Visit Rate")),tags$strong(": "), tags$i(tags$span(style="color:red",'Normalized mean for the PainandAbuse visit and line of business selected per 1000 people for Excellus counites')))
   ...
   conditionalPanel(
         condition = "input.dependency_grouping == 'Diagnosis'",

    absolutePanel(id = "controls", class = "panel panel-default", fixed = TRUE, draggable = TRUE, bottom = 0 , right = "auto", left = -5, top = "auto", width = 760, height= 10,
                 tags$p(tags$strong(tags$u("Indexed Diagnosis Visit Rate")),tags$strong(": "), tags$i(tags$span(style="color:red",'Normalized mean for the Diagnosis visit and line of business selected per 1000 people for Excellus counites')))
     ...
     conditionalPanel(
         condition = "input.dependency_grouping == 'PainAbuseDiagnosis'",
...
if(input$dependency_grouping=='PainandAbuse') {
  ...
  if(length(input$visit)==1)
    ...
    else if(length(input$visit)==5)
    {
      ...
      if(length(input$payer)==1)
      {
        ...
        df[,6]<- df[,6] + df[,13]+df[,20]+df[,27]+df[,34]
        df[,7]<- (df[,3]/df[,6])*1000
        return(df[,c(1:3,6,7)])
      }
      else
  ...
  output$map<- renderLeaflet({
    ...
    leaflet(map()) %>% addProviderTiles('HikeBike.HikeBike') %>% 
      ...
      addPolygons(fillColor = ~pal(map()@data[,42])
                  ,fillOpacity = 0.7
                  ,color =  'white'#"#03F"   #"black" 
                  ,weight = 1.5
                  ,dashArray='4'
                  ,popup = paste0(
                    "<span style='color: #2c7fb8'><strong>",map()@data$NAME_2,"</span></strong><br><strong>", 'Total number of Visits: ', "</strong>",prettyNum(map()@data[,41], big.mark = ","),"<br><strong>",'Number of Visits with Opioid Code: ', "</strong>",prettyNum(map()@data[,38], big.mark = ","),"<br><strong>",'Opioid Visit rate: ', "</strong>",round(map()@data$Actual.Opioid.Visit.Rate,3),"<br><strong>",'Indexed Opioid Visit rate: ', "</strong>",round(map()@data[,20],1),  "<br><strong>", "Excellus Region: " ,"</strong>", map()@data$region
                  )
